...
namespace Rookie.CustomerSite.Pages.Product
{
    public class ProductPageModel : PageModel
    {
        ...
        public async Task OnGetProductCategoryNameAsync(string categoryName)
        {
            var categoryCriteriaDto =
                new CategoryCriteriaDto {
                    ...
                    Page = 1,
                    Limit =
                        int.Parse(_config[ConfigurationConstants.PAGING_LIMIT])
                };
            var pageResponseDto =
                await _productService
                    .GetProductByCategoryAndPageAsync(categoryCriteriaDto);
            PagedResponseVM =
                _mapper
                    .Map<PagedResponseVM<ProductVM<ImageVM>>>(pageResponseDto);
        ...
        public async Task OnGetAsync(int? pageIndex)
        {
            ...
            var pageResponseDto =
                await _productService
                    .GetAllProductAndPageAsync(productCriteriaDto);
            PagedResponseVM =
                _mapper
                    .Map<PagedResponseVM<ProductVM<ImageVM>>>(pageResponseDto);
            await ShowCategoryName();
